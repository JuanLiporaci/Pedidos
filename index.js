...
// Intentar leer la variable de diferentes formas (para compatibilidad con Railway)
// Railway Runtime V2 puede tener problemas con variables de entorno
...
if (!TOKEN || TOKEN.trim() === '') {
  ...
  const envDebug = Object.keys(process.env)
    ...
    .sort();
  console.error('ENV_DEBUG_KEYS encontradas:', envDebug);
  
  // Verificar si la variable existe pero está vacía
  if (process.env.hasOwnProperty('TELEGRAM_BOT_TOKEN')) {
    const val = process.env.TELEGRAM_BOT_TOKEN;
    console.error('⚠️ TELEGRAM_BOT_TOKEN existe en process.env pero:');
    console.error('   - Tipo:', typeof val);
    console.error('   - Longitud:', val ? val.length : 0);
    console.error('   - Valor:', val === '' ? '(cadena vacía)' : val === undefined ? '(undefined)' : '(tiene valor pero no se lee correctamente)');
  } else {
    console.error('⚠️ TELEGRAM_BOT_TOKEN NO existe en process.env');
  }
  
  // Mostrar todas las variables de entorno (útil para debug)
  const allEnvKeys = Object.keys(process.env).sort();
  console.error('\n📋 Total de variables de entorno disponibles:', allEnvKeys.length);
  console.error('🔍 Variables relacionadas con TELEGRAM/BOT/TOKEN:');
  ...
  if (relatedKeys.length > 0) {
    relatedKeys.forEach(k => {
      ...
      console.error(`   - ${k}: ${val ? `(longitud: ${val.length})` : '(vacía/undefined)'}`);
    });
  } else {
    console.error('   (ninguna encontrada)');
  }
  
  console.error('\n💡 SOLUCIÓN:');
  
  // Verificar si es Railway Runtime V2
  if (process.env.RAILWAY_BETA_ENABLE_RUNTIME_V2) {
    console.error('\n🚨 RAILWAY RUNTIME V2 DETECTADO - Este es el problema más probable');
    console.error('   Railway Runtime V2 (beta) tiene bugs conocidos con variables de entorno');
    console.error('   SOLUCIÓN:');
    console.error('   1. Ve a Railway → Tu Servicio → Settings → Runtime');
    console.error('   2. Desactiva "Runtime V2" o cámbialo a "Runtime V1"');
    console.error('   3. O elimina la variable RAILWAY_BETA_ENABLE_RUNTIME_V2');
    console.error('   4. Haz un REDEPLOY completo');
    console.error('   5. Ver archivo SOLUCION_RUNTIME_V2.md para más detalles\n');
  }
  
  console.error('   Pasos generales:');
  console.error('   1. Verifica que la variable esté en el SERVICIO (no solo en el proyecto)');
  console.error('   2. Asegúrate de hacer un REDEPLOY después de agregar/modificar variables');
  console.error('   3. Verifica que el nombre sea exactamente: TELEGRAM_BOT_TOKEN');
  console.error('   4. En Railway: Servicio → Variables → Verifica que esté ahí');
  console.error('   5. Si está en "Shared Variables", muévela a "Service Variables"');
  console.error('\n⚠️ SI ERES COLABORADOR (no owner):');
  console.error('   - Las variables en "Shared Variables" pueden no funcionar para colaboradores');
  console.error('   - Necesitas que estén en "Service Variables" específicamente');
  console.error('   - Verifica tus permisos: Settings → Team → Tu rol debe ser Admin o Deploy');
  console.error('   - Si no tienes permisos, pide al owner que mueva las variables a "Service Variables"\n');
...
TOKEN = TOKEN.trim();
console.log('✅ TELEGRAM_BOT_TOKEN detectado correctamente (longitud:', TOKEN.length, 'caracteres)');
